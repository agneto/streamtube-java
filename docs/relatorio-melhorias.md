...
| Ordem | Item | Refs | Esforço |
|-------|------|------|---------|
| 1 | Revogar refresh tokens no reset de senha | 1.1 | Baixo |
| 2 | Publicar no Rabbit e enviar e-mails após o commit | 2.2, 2.3 | Médio |
| 3 | Quebrar a transação do processamento de vídeo | 2.1 | Médio |
| 4 | Perfil `prod` sem segredos default (fail-fast) | 1.3 | Baixo |
| 5 | Rate limit: proxy-aware + eviction + rotas faltantes | 1.2 | Médio |
| 6 | CI (build + testes + spotlessCheck) | 3.1 | Baixo |
| 7 | Catch-all no exception handler + contrato de erro único | 2.4, 6.1 | Baixo |
| 8 | Upgrade Spring Boot 3.5.x | 4.1 | Médio |
| 9 | Testes unitários dos use cases de auth/upload + worker | 5.1, 5.2 | Médio |
| 10 | Limites no upload presignado + CORS | 1.5, 1.4 | Médio |
| 11 | Métricas/observabilidade + healthcheck do worker | 3.2, 3.3 | Médio |
| 12 | Docker (cache de camadas, non-root, JVM flags), ArchUnit, JaCoCo | 3.4, 4.2, 1.7 | Baixo |
