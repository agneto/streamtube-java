...
class RateLimitingFilterTest {
  ...
  void returns429WithRetryAfterAndErrorEnvelopeWhenLimitExceeded() throws Exception {
    MockHttpServletResponse rejected = exhaustAndRepeat("POST", "/auth/login", "10.0.0.1");
  ...
  void limitsResetPasswordAndConfirmEmail() throws Exception {
    assertThat(exhaustAndRepeat("POST", "/auth/reset-password", "10.0.0.2").getStatus())
        .isEqualTo(429);
    assertThat(exhaustAndRepeat("GET", "/auth/confirm-email", "10.0.0.2").getStatus())
        .isEqualTo(429);
  }

  @Test
  void ipsHaveIndependentBuckets() throws Exception {
    assertThat(exhaustAndRepeat("POST", "/auth/login", "10.0.0.3").getStatus()).isEqualTo(429);
    assertThat(request("POST", "/auth/login", "10.0.0.4").getStatus()).isEqualTo(200);
  }

  @Test
  void unlimitedPathsAreNeverThrottled() throws Exception {
    for (int i = 0; i < 30; i++) {
      assertThat(request("GET", "/videos/some-slug", "10.0.0.5").getStatus()).isEqualTo(200);
      assertThat(request("POST", "/videos", "10.0.0.5").getStatus()).isEqualTo(200);
    }
  }
}
